...
/// 
/// ```
/// use egui_colors::{Colorix; ColorPreset};
...
impl Default for Colorix {
    fn default() -> Self {
        Self { tokens: ColorTokens::default(),
            theme: [ColorPreset::default(); 12],
            theme_index: 0,
...
impl Colorix {
    ...
    pub fn light_dark_toggle_button(&mut self, ctx: &egui::Context, ui: &mut egui::Ui) {
        ...
        if ctx.style().visuals.dark_mode {
            ...
            if ui
                .add(egui::Button::new(egui::RichText::new("☀").size(20.)).min_size(egui::Vec2::new(30., 30.)).frame(false))
...
/// ```
    pub fn themes_dropdown(&mut self, ctx: &egui::Context, ui: &mut egui::Ui, custom_themes: Option<(Vec<&str>, Vec<[ColorPreset; 12]>)>, custom_only: bool) {
        ...
        if let Some(custom) = custom_themes {
            ...
            if custom_only {
                ...
                combi_themes = THEMES.to_vec().into_iter().chain(themes.into_iter()).collect();
                combi_names = THEME_NAMES.to_vec().into_iter().chain(names.into_iter()).collect();
            }
        }
        else {
        ...
        .show_ui(ui, |ui| {
            for i in 0..combi_themes.len() {
                if ui.selectable_value(&mut self.theme, combi_themes[i], combi_names[i]).clicked() {
                    self.theme_index = i;
                    self.update_colors(ctx);
                };
            }
...
/// A widget with 12 dropdown menus of the UI elements ('ColorTokens') that can be set.
            ...
            for (i, label) in LABELS.iter().enumerate() {
                ...
                .show_ui(ui, |ui| {
                    for j in 0..dropdown_colors.len() {
                        if ui.selectable_value(&mut self.theme[i], dropdown_colors[j], DROPDOWN_TEXT[j]).clicked() {
    ...
    pub fn custom_picker(&mut self, ui: &mut egui::Ui) {
        if egui::color_picker::color_edit_button_hsva(ui, &mut self.scales.custom, egui::color_picker::Alpha::Opaque).changed() {
